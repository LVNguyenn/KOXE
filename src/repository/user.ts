...
const UserRepository = {
  ...
  async getUserById(user_id: string) {
    ...
    try {
      ...
      if (!userDb) {
        return FormatData("failed", "User not found", null, 200);
      }
  ...
  async getProfileById(userId: string) {
    if (!userId) return FormatData("failed", "Invalid information.");

    const userRepository = getRepository(User);

    try {
      const userDb = await userRepository.findOneOrFail({
        where: { user_id: userId, blocked: false },
      });
  ...
  async getProfileByUsername(username: string) {
    ...
    try {
      const userDb = await userRepository.findOneOrFail({
        where: { username: username, blocked: false },
      });
      const { password, ...others } = userDb;
  ...
  async getAllEmployeesBySalonId(data: any) {
    try {
      ...
      const userDb = await userRepository
        ...
        .innerJoin("user.salonId", "salon", "salon.salon_id = :salonId", {
          ...data,
        })
        .select([
          "user.user_id",
