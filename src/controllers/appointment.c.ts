...
const appointmentController = {
  ...
  get: async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (let app in appointDb) {
        appointDb[app].user = {
          fullname: appointDb[app].user.fullname,
          phone: appointDb[app].user.phone,
        };
  ...
  updateOne: async (req: Request, res: Response) => {
    ...
    const filteredObject: any = Object.fromEntries(
      Object.entries(updateObject).filter(([key, value]) => value !== undefined)
    );
    const appointmentRepository = getRepository(Appointment);
    ...
    try {
      ...
      const responeSalon: string =
        status == 1
          ? `${salonDb?.name} đã chấp thuận lịch hẹn của bạn.`
          : `${salonDb?.name} đã từ chối lịch hẹn của bạn.`;
      createNotification({
        to: salonId ? appointDb.user_id : appointDb.salon_id,
        description: salonId
          ? responeSalon
          : `${userDb?.fullname} đã chỉnh sửa thông tin mô tả của lịch hẹn với salon của bạn.`,
        types: "appointment",
        data: id,
      ...
      if (salonId)
        newLogs(
          salonId,
          `Employee ${req.user} updated appointment with id ${id} `
        );
  ...
  delete: async (req: Request, res: Response) => {
    ...
    const filteredObject = Object.fromEntries(
      Object.entries(deleteObject).filter(([key, value]) => value !== undefined)
    );
    ...
    try {
      ...
      if (recordToDelete?.date >= currentDate) {
        createNotification({
          ...
          description: salonId
            ? `Salon ${recordToDelete?.salon.name} đã hủy lịch hẹn với bạn.`
            : `User ${recordToDelete?.user.fullname} đã hủy lịch hẹn với salon của bạn.`,
          types: "appointment",
          avatar: salonId
            ? recordToDelete.salon.image
            : recordToDelete.user.avatar,
          isUser: !salonId,
        });
      ...
      if (salonId)
        newLogs(
          salonId,
          `Employee deleted appointment with custormer ${recordToDelete.user?.fullname} - ${recordToDelete.user?.user_id}`
        );
  ...
  createAppointmentBySalon: async (req: Request, res: Response) => {
    ...
    try {
      ...
      const appointmentRepository = getRepository(Appointment);
      let appoint = new Appointment();
      const apmDb = await appointmentRepository.save({
        ...appoint,
        date,
        salon_id: salonId,
        user_id: userRp?.data?.user_id,
        description,
        car_id: carId,
        from,
      });
